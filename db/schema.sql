...
CREATE VIEW public.quantity_trend_timeline AS
 WITH RECURSIVE time_spine AS (
         SELECT (date_trunc('week'::text, (min(t.min_date))::timestamp with time zone))::date AS week_starting,
            (date_trunc('week'::text, (CURRENT_DATE)::timestamp with time zone))::date AS max_date
